            ...
            try {
                ...
                if (coupon) {
                    ...
                    if (updateResult.modifiedCount === 0) {
                        console.warn(`Coupon ${couponCode} not updated for COD order. Reason: limit reached.`);
                    } else {
                        console.log(`Coupon ${couponCode} usage count updated for COD order.`);
                    }
                }
            } catch (error) {
                console.error('Error updating coupon usage for COD:', error);
            }
        }
        ...
        for (const dbProduct of dbProducts) {
            ...
            for (const cartProduct of cartProducts) {
                if (!dbProduct.isBlocked) {
                    ...
                    if (totalPrice >= 5000) {
                        return res.status(400).json({ success: false, message: 'Wallet payment is not available for orders above ₹5000. Please choose a different payment method.' });
                    }
        ...
        if (couponCode) {
            try {
                ...
                if (coupon) {
                    ...
                    if (updateResult.modifiedCount === 0) {
                        console.warn(`Coupon ${couponCode} not updated for Wallet order. Reason: limit reached.`);
                    } else {
                        console.log(`Coupon ${couponCode} usage count updated for Wallet order.`);
                    }
                }
            } catch (error) {
                console.error('Error updating coupon usage for Wallet:', error);
            }
        }
...
const verifyRazorPayOrder = async (req, res) => {
    try {
        if (!req.session.user) {
            return res.status(401).json({ success: false, message: 'User not logged in', redirectUrl: '/login' });
        }
        ...
        if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !order_id) {
            ...
            return res.status(400).json({ success: false, message: 'Missing payment fields', redirectUrl: `/payment-failure/${order_id}` });
        }
        ...
        if (expectedSignature !== razorpay_signature) {
            ...
            return res.status(400).json({ success: false, message: 'Invalid payment signature', redirectUrl: `/payment-failure/${order_id}` });
        }

        const order = await Order.findOne({ _id: order_id, user: userId });
        if (!order) {
            return res.status(404).json({ success: false, message: 'Order not found', redirectUrl: '/payment-failure' });
        }
        ...
        if (order.status !== 'Pending') {
            ...
            return res.status(200).json({ success: true, message: 'Order already processed', redirectUrl });
        }

    
        if (order.couponApplied && order.coupon.couponCode) {
            try {
                ...
                if (coupon) {
                    ...
                    if (updateResult.modifiedCount === 0) {
                        console.warn(`Coupon ${couponCode} usage count not updated. Reason: limit reached.`);
                    } else {
                        console.log(`Coupon ${couponCode} usage count successfully updated.`);
                    }
                }
            } catch (error) {
        ...
        return res.status(200).json({ success: true, message: 'Payment successful', redirectUrl: `/payment-success/${order._id}` });
        ...
        if (req.body.order_id) {
            ...
            return res.status(500).json({ success: false, message: 'Server error during verification', redirectUrl: `/payment-failure/${req.body.order_id}` });
        }
        return res.status(500).json({ success: false, message: 'Server error during verification', redirectUrl: '/payment-failure' });
    }
};
...
const paymentFailurePage = async (req, res) => {
    try {
        ...
        if (!order) {
            order = await Order.findOne({
                razorpayOrderId: orderIdentifier,
                user: req.session.user ?._id
            });
        }

        if (!order) {
            console.log('Order not found with identifier:', orderIdentifier);
